  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex gap-1 mb-3">
          {speedPresets.map((preset) => (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() =>
                setConfig({ ...config, animationSpeed: preset.value })
              }
      ...
      {config.text.animation !== "none" && (
        <fieldset>
          <legend className="text-xs text-gray-500 dark:text-gray-400 mb-2 block">
            Text Animation Range
          </legend>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="x-range-slider" className="text-xs text-gray-400">
                X Range
              </label>
              <input
                id="x-range-slider"
            ...
            <div>
              <label htmlFor="y-range-slider" className="text-xs text-gray-400">
                Y Range
              </label>
              <input
                id="y-range-slider"
      ...
      <div className="flex items-center justify-between">
        <label htmlFor="glow-toggle" className="text-sm dark:text-gray-300">
          Glow Effect
        </label>
        <motion.button
          id="glow-toggle"
          ...
          whileTap={{ scale: 0.95 }}
          onClick={() =>
            setConfig({ ...config, enableGlow: !config.enableGlow })
          }
          className={`w-12 h-6 rounded-full p-1 transition-colors ${
            config.enableGlow ? "bg-purple-500" : "bg-gray-300 dark:bg-gray-600"
          }`}
        >
          <motion.div
